...
Everything above assumes 10% of traffic turns left. That is the single assumption the result is
most sensitive to, so here is the range. The figure shown is the spread — the gap between the
unlucky trip and the typical one — at 5%, 10% and 15%.

![Turn-share sensitivity](turn_share_sensitivity_20260831_1800.png)
...
**The four-lane road scales with both.** At 700 veh/h its spread runs from +8.0 s at a 5% turn
share to +14.9 s at 15%, and the share of drivers held up behind a left-turner goes from 4.3% to
16.4%.

**The turning percentage decides whether this is an average-speed argument at all.** At a 5% turn
share the four-lane road is the *quicker* of the two on average — 23.4 s against 24.0 s at
700 veh/h — because two lanes let a fast driver past a slow one and there is barely anything to get
stuck behind. At 10% they are level. At 15% the three-lane road wins outright by more than two seconds.
 It is the more reliable road at every share tested. **So if the real driveway
counts on this corridor are nearer 5% than 15%, the case to make is reliability and safety, not
travel time.**

One case runs the other way from how the capacity argument is usually put: at **15% turning and
850 veh/h**, the four-lane road stops serving its demand — 754 vehicles an hour against the
three-lane road's 849 — because left-turners waiting for a gap tie up one of its two lanes often
enough to cost more capacity than the diet does.

## Does the answer depend on which hours you simulate?

Every figure here averages twelve independent simulated hours. It used to average six, so the
obvious question is whether the extra six changed anything. They did not:

| At 700 veh/h, each direction | 6 hours | 12 hours | change |
|---|---:|---:|---:|
| 4-lane average travel time | 27.28 s | 27.36 s | +0.08 s |
| 3-lane average travel time | 27.02 s | 27.00 s | −0.02 s |
| 4-lane 95th percentile | 38.38 s | 39.10 s | +0.73 s |
| 3-lane 95th percentile | 29.12 s | 29.10 s | −0.02 s |
| **4-lane spread** | **+11.09 s** | **+11.74 s** | **+0.65 s** |
| **3-lane spread** | **+2.11 s** | **+2.10 s** | **−0.01 s** |

Across all sixteen sweep points, no average travel time moved by more than 0.08 s and no
throughput figure by more than about six vehicles an hour. The largest movement anywhere was the
four-lane spread at the two busiest volumes, which grew by about 0.6 s — that is, the extra hours
made the four-lane road look slightly *worse*, not better. Nothing about the conclusion depends on
which hours were drawn.

Individual hours, though, vary a great deal, which is the reason for averaging in the first place:

| Spread across the twelve individual hours | 4 lanes | 3 lanes + turn lane |
|---|---:|---:|
| 400 veh/h | 2.2 – 7.1 s | 2.2 – 2.8 s |
| 550 veh/h | 3.1 – 10.8 s | 2.1 – 2.6 s |
| 700 veh/h | 8.8 – 14.9 s | 1.8 – 2.5 s |
| 850 veh/h | 13.8 – 17.4 s | 1.6 – 2.0 s |

At 700 veh/h a single unlucky four-lane hour would have shown +14.9 s and a lucky one +8.8 s. The
three-lane road never leaves a 0.7-second band at any volume — the low variance between hours is
itself part of the reliability finding, not just a statistical convenience.

The animated explainer stores all twelve hours and lets you replay whichever you choose, so the
picture is fixed and inspectable rather than reshuffling on each load.
...
A single through lane discharging at a two-second saturation headway carries about 1,800 veh/h
*while it has green*. Multiply by the split and you have the ceiling: roughly 1,080 veh/h at 60%
green, 1,170 at 65%, but only about 900 at half the cycle. The four-lane road, with two lanes at the stop bar,
has twice that headroom and never notices the difference.
...
| **60% green (54 s of a 90 s cycle)** | **858** | **857** | **24.8 s** | **8 veh** |
...
half the cycle it falls about 100 vehicles an hour short and a queue builds behind the stop bar.

### Would a more generous split change the answer?

The model uses 60%. Running the same sweep at the more generous 65% is the check worth having,
because someone will ask whether the result depends on a favorable split. Both cross-sections
serve their full demand at either setting, 850 included, with a residual queue of under ten
vehicles behind the stop bar. Everyone's trip is about a third of a second shorter at 65%, on both
roads. What changes is the spread:
...
less room to get around a left-turner — and gives a road with nothing blocking it nothing to
notice. So the case does not rest on a generous split: between 60% and 65% the diet is the more
reliable road either way, and by more at the 60% this model assumes.
...
850 level is about 945 veh/h including the turners, comfortably under the first two and over the
third.
...
them. Throughput is unchanged either way.
...
driver more gaps to escape into. Drivers held up behind a left turn fall from 11.0% to 7.2% at
700 veh/h, and from 17.3% to 12.3% at 850.
...
So the question to bring to the corridor's signal timing plan is a specific and answerable one:
**how much of the cycle do the intersections bracketing this block give the mainline?** At
two-thirds, the lane count stops mattering. At half, the ceiling is real. **This is the question to bring to the
corridor's actual signal timing.** If the intersections bracketing a block give the mainline about
two-thirds of the cycle, one through lane carries the volumes in this study comfortably. If they
give it half, the ceiling is real and worth knowing before anyone promises otherwise.
...
The model was also run with every left-turner replaced by a through vehicle — same number of
vehicles at the stop bar, none of them turning. On four lanes, adding the left turns back costs
through traffic **2.2 seconds per vehicle** at the 700 level. On three lanes the same change is
worth **−0.3 seconds**: through traffic is slightly *better* off with left-turners present,
because they leave the road at their driveways and stop consuming the single through lane.

Both figures contain that same relief effect, so the difference between them — roughly nine
seconds — is the cleaner measure of what the center lane buys. The direct comparison in the tables
above is the more useful number to quote; this run is here because it rules out the objection that
the difference is really about lane count rather than left turns.
...
| `turn_share_sensitivity_20260831_1800.png` | Turn-share figure — full edition only |
